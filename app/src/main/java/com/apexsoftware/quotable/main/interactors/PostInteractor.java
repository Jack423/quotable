...
public class PostInteractor {
    ...
    public void createPost(Post post, OnPostCreatedListener onPostCreatedListener) {
        if (post.getId() == null) {
            ...
            for (String key : objectMap.keySet()) {
                ...
                if (obj instanceof Map) {
                    ...
                    if (!hasComplain) {
                        Post post = new Post();
                        post.setId(key);
                        post.setQuote((String) mapObj.get("quote"));
    ...
    public ValueEventListener searchPostsByQuote(String searchText, OnDataChangedListener<Post> onDataChangedListener) {
        DatabaseReference reference = databaseHelper.getDatabaseReference().child(DatabaseHelper.POSTS_DB_KEY);
        ValueEventListener valueEventListener = getSearchQuery(reference,"quote", searchText).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                PostListResult result = parsePostList((Map<String, Object>) dataSnapshot.getValue());
                onDataChangedListener.onListChanged(result.getPosts());
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                LogUtil.logError(TAG, "searchPostsByQuote(), onCancelled", new Exception(databaseError.getMessage()));
            }
        });
